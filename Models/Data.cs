...
namespace ReestrForm
{
    ...
    "Port=6543;" +
    "Database=postgres;" +
    "Timeout=1;" +              // Час очікування на підключення
    "CommandTimeout=1;" + "Pooling=true;Maximum Pool Size=50;";
        ...
        public static void DeleteData<T>(string tableName, string keyPropertyName, object keyValue)
        {
            using var conn = new NpgsqlConnection(ConnectionString);
            conn.Open();

            string query = $"DELETE FROM {tableName} WHERE {keyPropertyName} = @keyValue;";
            ...
            {
                Console.WriteLine($"Error executing DELETE:\n{query}\nException: {ex.Message}");
                throw;
            }
        }
        ...
        public static List<Dictionary<string, object?>> ReadRows(string query)
        {
            using var conn = new NpgsqlConnection(ConnectionString);
            conn.Open();
            using var cmd = new NpgsqlCommand(query, conn);
        ...
        public static async Task<List<Dictionary<string, object?>>> ReadRowsAsync(string query)
        {
            using var conn = new NpgsqlConnection(ConnectionString);
            await conn.OpenAsync();
            using var cmd = new NpgsqlCommand(query, conn);
        ...
        public static string? GetRateNameById(int rateId)
        {
            string query = $"SELECT name FROM rates WHERE id = {rateId} LIMIT 1;";
            var result = ReadQuery(query);

            if (result.Count > 0 && result[0].TryGetValue("name", out var nameValue))
            {
        ...
        public static Task<List<Dictionary<string, object?>>> ReadQueryAsync(string query) => ReadRowsAsync(query);
    }
}
